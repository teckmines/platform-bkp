...
function VolumeReport() {
  ...
  const handleTripInfo = (value) => {
    ...
    if(value==='contractor'){
      ...
      setRowData(['Overweight','Underweight']);
    }
    if(url!=``){
    axios({
    method: 'GET',
  ...
  const handleChangeLast = (value) =>{
    ...
    params: newdata[0],
      ...
      if(category === 'contractor' || category === 'truck'){
      if(response.data.data.volume.length>0&&response.data.data.volume){
        const getValues = response.data.data.volume.map((obj) => {
          console.log(obj)
      ...
      setGraphData(getValues)
        ...
        if(response.data.data.num_of_trucks.length>0&&response.data.data.num_of_trucks){
          const getValues = response.data.data.num_of_trucks.map((obj) => {
            console.log(obj)
            return {...obj, value: parseFloat(obj.value), date: obj.date}
        })
        console.log(getValues)
 ...
 const handleLogout = () =>{
    ...
    return (
        ...
        <Grid container spacing={1} style={{background:'#bfe1f3', borderTopLeftRadius: '20px', borderTopRightRadius: '20px', marginLeft: '0.01rem' , width: 'auto'}}>
            <Grid item xs={6}>
                <Typography className='formHeader'>Volume Report
                </Typography>
                          ...
                          <label>Category</label>
                            <FormControl fullWidth>
                                <Select size="small" labelId="demo-simple-select-label" placeholder="Truck" id="demo-simple-select" value={category} onChange={handleCategoryChange} >
                                    ...
                                    <MenuItem value="weight">Overweight/Underweight</MenuItem>
                                </Select>
                            </FormControl>
                        </Grid>
                        <Grid item xs={3}>
