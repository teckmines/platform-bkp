...
function VolumeCapture() {
    ...
    const handleCapture = () => {
        const url = `http://${process.env.REACT_APP_DEVICE_MANAGER}/deviceManager/loadSense/register`;
        const newdata = {
            "truckId":blast,
            "deviceId":1
        }
    ...
    const handleDownload = () => {
        axios.get(`http://${process.env.REACT_APP_DEVICE_MANAGER}/deviceManager/loadSense/pointcloud/fetch`,{
          params:{
            "truckId":blast
          }
        },{responseType: "blob"})
    ...
    const handleUpload = (data) => {
        console.log(blast)
        let url = `http://${process.env.REACT_APP_DEVICE_MANAGER}/deviceManager/loadSense/processBody?truckId=${blast}`
        if(mine=='head'){
            url = `http://${process.env.REACT_APP_DEVICE_MANAGER}/deviceManager/loadSense/processHead?truckId=${blast}`
        }else if(mine=='truck') {
            url = `http://${process.env.REACT_APP_DEVICE_MANAGER}/deviceManager/loadSense/processTruck?truckId=${blast}`
        }
    ...
    return (
        ...
        <Grid container spacing={1} style={{background:'#bfe1f3', borderTopLeftRadius: '20px', borderTopRightRadius: '20px', marginLeft: '0.01rem' , width: 'auto'}}>
            ...
            <Grid item xs={1}>
                ...
                <Button color="primary" variant="contained" style={{marginLeft: '2rem', marginTop: '1rem',marginBottom: '1rem', color: '#fff', background: 'linear-gradient(to right, #000046, #1cb5e0)' , borderRadius: '20px'}} onClick={handleCapture} >Capture</Button>
                <Button color="primary" variant="contained"style={{marginLeft: '1rem', marginTop: '1rem',marginBottom: '1rem', color: '#fff', background: 'linear-gradient(to right, #000046, #1cb5e0)' , borderRadius: '20px'}}  disabled={blast!=''?false:true} onClick={handleDownload} startIcon={<DownloadIcon />} >Download</Button>
                <input type="file" accept=".ply" ref={hiddenFileInput} hidden onChange={handleUpload}  onClick={(event)=> { event.target.value = null }}/> <Button color="primary" variant="contained"style={{marginRight: '2rem', marginTop: '1rem',marginBottom: '1rem', color: '#fff', background: 'linear-gradient(to right, #000046, #1cb5e0)' , borderRadius: '20px',float: "right"}}  disabled={mine!=''?false:true} onClick={handleClick} startIcon={<UploadIcon />} >Upload</Button><br></br>
