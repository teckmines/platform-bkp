...
function Blast(props) {
  // console.log(props.user)
    ...
    const [minerowData, setMineRowData] = useState([]);
    ...
    useEffect(() =>{
      ...
      const url1 = `/getMineInfo`;
      axios.get(url1)
      .then(response => {
          console.log(response.data.data)
          setMineRowData(response.data.data);
      }).catch(error => {
          console.log(error)
      })
  },[token])
    ...
    return (
        <Box>           
            ...
            <Grid container spacing={1} style={{padding:'2.5rem'}}>
                <Grid item xs={6}>
                    <Typography className='pageHeader'> Primary Blast Details</Typography>
                </Grid>
                    <Grid item xs={5}>
            ...
            {!addform ? <Paper className='tablepaper'>
                    ...
                    <TableContainer component={Paper}>
                        <Table sx={{ minWidth: 700 }} aria-label="customized table">
                            <TableHead>
                            <TableRow>
                                <StyledTableCell>Blast ID</StyledTableCell>
                                <StyledTableCell>Blast Pattern</StyledTableCell>
                                <StyledTableCell>Blasting Incharge</StyledTableCell>
                                <StyledTableCell>Mine No</StyledTableCell>
                                <StyledTableCell>Blasting Date & Time</StyledTableCell>
                                <StyledTableCell>Drilling Pattern</StyledTableCell>
                            </TableRow>
                            </TableHead>
                            <TableBody>
                            {rowData.map((row) => (
                                ...
                                <StyledTableCell>{row.blast_pattern}</StyledTableCell>
                                <StyledTableCell>{row.blasting_incharge}</StyledTableCell>
                                <StyledTableCell>{row.mine_reg_no}</StyledTableCell>
                                <StyledTableCell>{ new Date(row.blast_date_time).toString()}</StyledTableCell>
                                <StyledTableCell>{row.drilling_pattern}</StyledTableCell>
                ...
                <Paper className='formpaper'>
                    ...
                    <BlastForm callback={handleCallback} editData={editData} chargeData={chargeData} rowData={minerowData} />
                </Paper>
                }
            </Box>
        </Box>
